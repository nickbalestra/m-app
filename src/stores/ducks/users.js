...
const cycleFetchUsers = sources => {
  function networking(sources) {
    ...
    const usersOffset$ = sources.STATE.map(({ users }) => users.users.length);
  ...
  function intent(sources) {
    const action$ = sources.HTTP.select(FETCH_USERS)
...
const cycleInitUsers = sources => {
  function intent(sources) {
    return sources.ACTION.filter(({ type }) => type === INIT_USERS)
...
export const cycle = combineCycles(cycleFetchUsers, cycleInitUsers);
